...
Counts what a capture contains: packets, messages, heartbeats, and a tally per message type.
Anything the parser could not identify is counted separately, so a feed carrying something
unexpected shows up as a non-zero unknown count rather than passing quietly.
