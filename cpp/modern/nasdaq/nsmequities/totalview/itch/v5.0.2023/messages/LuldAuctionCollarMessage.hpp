...
struct LuldAuctionCollarMessage {
    ...
    static const LuldAuctionCollarMessage* parse(const SequencedDataPacket* header) {
        return reinterpret_cast<const LuldAuctionCollarMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SequencedDataPacket));
    }
};
