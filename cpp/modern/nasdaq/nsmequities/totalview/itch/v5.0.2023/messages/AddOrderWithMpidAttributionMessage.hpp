...
struct AddOrderWithMpidAttributionMessage {
    ...
    static const AddOrderWithMpidAttributionMessage* parse(const MessageHeader* header) {
        return reinterpret_cast<const AddOrderWithMpidAttributionMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(MessageHeader));
    }
};
