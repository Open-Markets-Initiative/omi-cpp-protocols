...
struct SecurityDirectoryMessage {
    ...
    static const SecurityDirectoryMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const SecurityDirectoryMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
