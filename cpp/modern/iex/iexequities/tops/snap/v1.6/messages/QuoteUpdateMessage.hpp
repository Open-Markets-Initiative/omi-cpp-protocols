...
struct QuoteUpdateMessage {
    ...
    static const QuoteUpdateMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const QuoteUpdateMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
