...
struct TradingStatusMessage {
    ...
    static const TradingStatusMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const TradingStatusMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
