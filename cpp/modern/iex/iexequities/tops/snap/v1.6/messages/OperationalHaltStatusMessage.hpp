...
struct OperationalHaltStatusMessage {
    ...
    static const OperationalHaltStatusMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const OperationalHaltStatusMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
