...
struct RetailLiquidityIndicatorMessage {
    ...
    static const RetailLiquidityIndicatorMessage* parse(const MessageHeader* header) {
        return reinterpret_cast<const RetailLiquidityIndicatorMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(MessageHeader));
    }
};
