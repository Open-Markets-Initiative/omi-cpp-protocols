...
struct AuctionInformationMessage {
    ...
    static const AuctionInformationMessage* parse(const MessageHeader* header) {
        return reinterpret_cast<const AuctionInformationMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(MessageHeader));
    }
};
