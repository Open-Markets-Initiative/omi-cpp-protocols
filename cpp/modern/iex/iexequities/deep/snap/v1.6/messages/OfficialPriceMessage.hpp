...
struct OfficialPriceMessage {
    ...
    static const OfficialPriceMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const OfficialPriceMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
