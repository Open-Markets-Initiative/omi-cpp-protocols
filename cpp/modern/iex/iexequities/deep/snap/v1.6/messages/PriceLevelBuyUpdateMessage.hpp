...
struct PriceLevelBuyUpdateMessage {
    ...
    static const PriceLevelBuyUpdateMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const PriceLevelBuyUpdateMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
