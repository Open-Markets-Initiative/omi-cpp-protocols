...
struct TradeReportMessage {
    ...
    static const TradeReportMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const TradeReportMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
