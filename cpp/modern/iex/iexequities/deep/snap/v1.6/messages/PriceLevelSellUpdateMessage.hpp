...
struct PriceLevelSellUpdateMessage {
    ...
    static const PriceLevelSellUpdateMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const PriceLevelSellUpdateMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
