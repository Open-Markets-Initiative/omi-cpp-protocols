...
struct TradeBreakMessage {
    ...
    static const TradeBreakMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const TradeBreakMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
