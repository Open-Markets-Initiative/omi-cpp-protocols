...
struct SecurityEventMessage {
    ...
    static const SecurityEventMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const SecurityEventMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
