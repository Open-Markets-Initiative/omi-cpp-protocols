...
struct ShortSalePriceTestStatusMessage {
    ...
    static const ShortSalePriceTestStatusMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const ShortSalePriceTestStatusMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
