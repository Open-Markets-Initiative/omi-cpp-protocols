...
struct OrderModifyMessage {
    ...
    static const OrderModifyMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const OrderModifyMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
