...
struct TradeMessage {
    ...
    static const TradeMessage* parse(const SnapshotDataMessage* header) {
        return reinterpret_cast<const TradeMessage*>(reinterpret_cast<const std::byte*>(header) + sizeof(SnapshotDataMessage));
    }
};
