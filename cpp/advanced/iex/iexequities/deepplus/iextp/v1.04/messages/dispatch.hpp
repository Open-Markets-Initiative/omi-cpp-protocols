...
void dispatch(Handler& handler, const std::byte* buffer, std::size_t length, std::uint64_t packet_receive_time, const iextp_header& transport) {
    ...
    switch (header->message_type.get().value()) {
        ...
        case message_type::enum_type::retail_liquidity_indicator_message:
            handler.on_message(*retail_liquidity_indicator_message::parse(buffer), packet_receive_time, transport);
            break;
        ...
        case message_type::enum_type::order_modify_message:
            handler.on_message(*order_modify_message::parse(buffer), packet_receive_time, transport);
            break;
        ...
        case message_type::enum_type::clear_book_message:
            handler.on_message(*clear_book_message::parse(buffer), packet_receive_time, transport);
            break;
        default:
