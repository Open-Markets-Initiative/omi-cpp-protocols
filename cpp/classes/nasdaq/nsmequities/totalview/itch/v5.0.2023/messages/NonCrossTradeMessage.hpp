...
class NonCrossTradeMessage : public Message {
  public:
    // The code that selects this message
    static constexpr MessageCode message_type = MessageType::NonCrossTradeMessage;
    // Bytes of this message on the wire
    static constexpr std::size_t wire_size = 43;

    NonCrossTradeMessage() = default;
    NonCrossTradeMessage(std::uint16_t stock_locate, std::uint16_t tracking_number, std::chrono::nanoseconds timestamp, std::uint64_t order_reference_number, BuySellIndicator buy_sell_indicator, std::uint32_t shares, const std::string& stock, Decimal price, std::uint64_t match_number);
    ...
    bool operator==(const NonCrossTradeMessage& other) const;
    bool operator!=(const NonCrossTradeMessage& other) const;
