...
class OperationalHaltMessage : public Message {
  public:
    // The code that selects this message
    static constexpr MessageCode message_type = MessageType::OperationalHaltMessage;
    // Bytes of this message on the wire
    static constexpr std::size_t wire_size = 20;

    OperationalHaltMessage() = default;
    OperationalHaltMessage(std::uint16_t stock_locate, std::uint16_t tracking_number, std::chrono::nanoseconds timestamp, const std::string& stock, MarketCode market_code, OperationalHaltAction operational_halt_action);
    ...
    bool operator==(const OperationalHaltMessage& other) const;
    bool operator!=(const OperationalHaltMessage& other) const;
