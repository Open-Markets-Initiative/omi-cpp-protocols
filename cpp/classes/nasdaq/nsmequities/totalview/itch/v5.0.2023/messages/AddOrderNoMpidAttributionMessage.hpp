...
class AddOrderNoMpidAttributionMessage : public Message {
  public:
    // The code that selects this message
    static constexpr MessageCode message_type = MessageType::AddOrderNoMpidAttributionMessage;
    // Bytes of this message on the wire
    static constexpr std::size_t wire_size = 35;

    AddOrderNoMpidAttributionMessage() = default;
    AddOrderNoMpidAttributionMessage(std::uint16_t stock_locate, std::uint16_t tracking_number, std::chrono::nanoseconds timestamp, std::uint64_t order_reference_number, BuySellIndicator buy_sell_indicator, std::uint32_t shares, const std::string& stock, Decimal price);
    ...
    bool operator==(const AddOrderNoMpidAttributionMessage& other) const;
    bool operator!=(const AddOrderNoMpidAttributionMessage& other) const;
