...
// Carries one complete Tops feed message wrapped with an IexTp header providing Tops feed
...
class SnapshotDataMessage : public Message {
  public:
    ...
    // Iex Tp Header: IexTp header for the wrapped Tops feed message, see the Iex Transport
    // specification
    ...
    // Iex Tp Message Length: Length of the wrapped Tops message in bytes not including this
    // field
    std::uint16_t iex_tp_message_length() const;
    void set_iex_tp_message_length(std::uint16_t value);

    // Iex Tp Message Type: Code identifying the wrapped Tops message type
