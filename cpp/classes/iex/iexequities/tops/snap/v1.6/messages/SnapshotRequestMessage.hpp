...
// The Snapshot Request message is sent from the client to the Tops Snap server to authenticate
...
class SnapshotRequestMessage : public Message {
  public:
    ...
    // Channel Id: Channel identifier from the Tops feed IexTp header
    std::uint32_t channel_id() const;
    void set_channel_id(std::uint32_t value);

    // Session Id: Session identifier from the Tops feed IexTp header
