...
// The Snapshot Request message is sent from the client to the Deep Plus Snap server to
// authenticate and request a Snapshot Response
class SnapshotRequestMessage : public Message {
  public:
    ...
    // Channel Id: Channel identifier from the Deep Plus feed IexTp header
    std::uint32_t channel_id() const;
    void set_channel_id(std::uint32_t value);

    // Session Id: Session identifier from the Deep Plus feed IexTp header
