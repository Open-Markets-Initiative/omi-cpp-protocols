...
// A non-displayed order on the book that executed against another non-displayed order on the
// book
class TradeMessage : public IexTpMessageData {
  public:
    // The code that selects this message
    static constexpr IexTpMessageDataCode message_type = IexTpMessageType::TradeMessage;
    // Bytes of this message on the wire
    static constexpr std::size_t wire_size = 37;

    TradeMessage() = default;
    TradeMessage(const SaleConditionFlags& sale_condition_flags, std::chrono::nanoseconds timestamp, const std::string& symbol, std::uint32_t size, Decimal price, std::uint64_t trade_id);
